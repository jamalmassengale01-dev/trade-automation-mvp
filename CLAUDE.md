...
**How to test the next idea:** `research/entry-screen/`. Paste an entry condition
into `entry-edge-tester.pine`, export the trade list, run `edge.py` on it. The
README explains the protocol; the short version is that significance alone is not
enough — an effect must also survive a split-half stability check and clear
commission, and both of those have killed a result that passed the first.

## ⚠ FIRST REPLACEMENT SEARCH, 17 September 2026: NINE IDEAS, NO SURVIVOR

The obvious next question after "GB LIVE has no edge" was "what replaces it". A
first attempt has been made and failed. Full record in
`research/entry-screen/RESULTS.md`; do not repeat these nine.

Nine entry conditions, generated by GPT across ten hypothesis families, screened
on MNQ 5-minute bars over Sep 2023 – Mar 2025 against a **+2.6 sigma** bar set
before any result was seen:

```
high-volume impulse          +2.36   split-half -0.20/+3.49, EV below commission
same-interval-yesterday      +1.39
close-location persistence   +1.26
Treasury -> MNQ              +1.23
opening -> closing half-hour +1.03
20-bar momentum              +0.00
VIX stress                   -0.47   (long leg structurally dead; short-only)
ATR shock reversal           -1.77
ES -> NQ convergence         untestable, 6 trades in 18 months
turn of month                withdrawn, ~36 observations in 3 years
```

One result above 2 sigma, and its entire effect lived in the second half of its
own discovery window while its EV sat below commission.
...
**What this does not establish.** Not that no intraday edge exists. Nine
hypotheses is a small sample of a large space; all were generated in two sittings
by one model, tested on one instrument at one timeframe, and constrained to a few
lines of OHLCV Pine. That last constraint excludes most of what professional
intraday strategies actually use — order book depth, cross-sectional breadth,
options positioning, dealer gamma — and it ruled those out before a single test
ran. A second batch drawn from the same constrained pool will likely produce the
same table.

**One measured fact worth carrying forward.** Cost drag falls sharply with
timeframe and session: ATR-based stops of ~15 points on 5-minute all-session
give commission at 3.5% of R, while ~48 points on RTH-only or 45-minute bars give
1.4%. High-volume impulse was killed partly BY cost, not only by weak signal. A
declared second pass at a higher timeframe is cheap, but it must be declared over
all ideas at once — testing nine ideas on two timeframes raises the bar from 2.6
...
├── research/entry-screen/             ← does an entry predict, or is it a coin flip?
│   ├── entry-edge-tester.pine         ← paste an entry condition, export the trades
│   ├── edge.py                        ← scores the export against 1/(1+T)
│   ├── README.md                      ← the protocol, and why win rate is not it
│   └── RESULTS.md                     ← nine ideas screened, none survived
